...
def lambda_handler(event, context):
    ...
    return {
        ...
        "body": json.dumps({
            "message": "aaaaa",
            "total timesteps taken": len(population[population[:, 6] == 3]),
            "total dead": len(population[population[:, 6] == 3]),
            "total recovered": len(population[population[:, 6] == 2]),
            "total infected": len(population[population[:, 6] == 1]),
            "total infectious": len(population[(population[:, 6] == 1) | (population[:, 6] == 4)]),
            "total unaffected": len(population[population[:, 6] == 0])
